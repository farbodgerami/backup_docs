...
#!/bin/bash
export PGPASSWORD=<YOUR_DATABASE_PASSWORD>
docker exec apppostgresql pg_dump -FC  -U postgres -h 127.0.0.1 -p 5432 -d mydb > /home/farbod/postgresbackup/$(date +'%Y_%m_%d_%H_%M_%S')_mydb.sql.dump

docker exec apppostgresql pg_restore -FC  -U postgres -h 127.0.0.1 -p 5432 -d mydb > /home/farbod/postgresbackup/$(date +'%Y_%m_%d_%H_%M_%S')_mydb.sql.dump
